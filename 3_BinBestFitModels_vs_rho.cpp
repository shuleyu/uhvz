...
int main() {
    ...
    // read in minerology data.
    ifstream fpin("diamond.txt");
    vector<double> diamond_dvs, diamond_drho, diamond_vol;
    double v1, v2, v3;
    while (fpin >> v1 >> v2 >> v3){
        diamond_drho.push_back(v1);
        diamond_dvs.push_back(v2);
        diamond_vol.push_back(v3);
    }
    fpin.close();
    ...
    for (size_t i=0; i<binInfo.NRow(); ++i) {
        ...
        for (size_t j = 0; j < modelingResult.NRow(); ++j) {
            ...
            if (cq < premCQ + dCQThreshold) {
                badData[k].push_back({properties[1], properties[2], 0.05});
            }
            else {
                goodData[k].push_back({properties[1], properties[2], cq/6.0});
            }
        ...
        for (size_t k = 0; k < rhoDimensions.size(); ++k) {
            GMT::psxy(outfile, badData[k],  "-JX" + to_string(plotWidth * 0.8 ) + "i/" + to_string(plotHeight * 0.8) + "i -R-30.5/20.5/-5.5/10.5 -Sx -W1p," + penColor[k] + " -O -K");
        }
        for (size_t k = 0; k < rhoDimensions.size(); ++k) {

            GMT::psxy(outfile, goodData[k], "-JX" + to_string(plotWidth * 0.8 ) + "i/" + to_string(plotHeight * 0.8) + "i -R-30.5/20.5/-5.5/10.5 -Sc -W1p," + penColor[k] + " -O -K");
        }

        GMT::psxy(outfile, diamond_dvs, diamond_drho, "-JX" + to_string(plotWidth * 0.8 ) + "i/" + to_string(plotHeight * 0.8) + "i -R-30.5/20.5/-5.5/10.5 -W2p,orange -O -K");


        GMT::psbasemap(outfile, "-J -R -Bxa5 -Bya2.5 -BWSne -O -K");

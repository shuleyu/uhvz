...
if (modelName.find("Lamella") == string::npos && modelName.find("PREM") == string::npos) {
    ...
    for (auto &item:Data) {
        ...
        // cut and flip and dump the files:
//         item.dataStack.Mask(15, 30);
//         item.dataStack.FlipReverseSum(15);
//         item.dataStack.ShiftTime(-15);
//         item.dataStack.CheckAndCutToWindow(-14.975, 0);
//         item.dataStack.OutputToFile("/Users/shuleyu/data/DataStack_Bin" + to_string(item.binN) +"_" + item.bestModel.substr(0, 4) + item.bestModel.substr(item.bestModel.size() - 4, 4) + ".txt");
// 
//         item.modelStack.Mask(15, 30);
//         item.modelStack.FlipReverseSum(15);
//         item.modelStack.ShiftTime(-15);
//         item.modelStack.CheckAndCutToWindow(-14.975, 0);
//         item.modelStack.OutputToFile("/Users/shuleyu/data/ModelStack_Bin" + to_string(item.binN) +"_" + item.bestModel.substr(0, 4) + item.bestModel.substr(item.bestModel.size() - 4, 4) + ".txt");
    }
    GMT::MoveReferencePoint(outfile,"-Y-1i");
